...
DEFAULT_QUERIES = [
    ...
    # Requested rainfall/flood monitoring locations
    "Imus Cavite flood today",
    "Bacoor Cavite flood today",
    "Noveleta Cavite flood today",
    "Dasmarinas Cavite flood today",
    "Victoria Laguna flood today",
    "Morong Rizal flood today",
    "Iba Zambales flood today",
...
RAIN_STATIONS_REQUESTED = {
    "Imus, Cavite": (14.4189, 120.9312),
    "Bacoor, Cavite": (14.4594, 120.9401),
    "Noveleta, Cavite": (14.4333, 120.8833),
    ...
    "Casandig, Gandara, Samar": (12.0371, 124.8247),
}
...
SNAPSHOT_REGIONS = {
    ...
    "Visayas": [
        ...
        "Dumaguete City, Negros Oriental", "Casandig, Gandara, Samar",
    ],
    "Mindanao": [
...
LOCATION_GAZETTEER = {
    ...
    # Requested monitoring locations and common aliases
    "imus, cavite": (14.4189, 120.9312),
    "imus cavite": (14.4189, 120.9312),
    "imus": (14.4189, 120.9312),
    "bacoor, cavite": (14.4594, 120.9401),
    "bacoor cavite": (14.4594, 120.9401),
    "bacoor": (14.4594, 120.9401),
    "noveleta, cavite": (14.4333, 120.8833),
    "noveleta cavite": (14.4333, 120.8833),
    "noveleta": (14.4333, 120.8833),
    "dasmariñas, cavite": (14.3270, 120.9370),
    "dasmariñas cavite": (14.3270, 120.9370),
    "dasmarinas cavite": (14.3270, 120.9370),
    "dasmariñas": (14.3270, 120.9370),
    "dasmarinas": (14.3270, 120.9370),
    "victoria, laguna": (14.2316, 121.3278),
    "victoria laguna": (14.2316, 121.3278),
    "morong, rizal": (14.5151, 121.2380),
    "morong rizal": (14.5151, 121.2380),
    ...
    "casiguran, aurora": (16.2041, 122.0400),
    "casiguran aurora": (16.2041, 122.0400),
    "dumaguete city": (9.3054, 123.3080),
    "dumaguete": (9.3054, 123.3080),
    "negros oriental": (9.3054, 123.3080),
    "casandig, samar": (12.0371, 124.8247),
    "casandig samar": (12.0371, 124.8247),
    "casandig": (12.0371, 124.8247),
...
if show_rain_layer:
    if rainfall_coverage == "Marikina + Rizal Rain Watch":
        ...
        selected_stations = RAIN_STATIONS_NATIONWIDE
...
if show_rain_layer and not rainfall_df.empty:
    ...
    elif rainfall_coverage == "Nationwide key cities":
        available_snapshot_regions = [
            "Metro Manila",
            "North Luzon",
            "South Luzon",
...
with left:
        ...
        for _, rrow in rainfall_df.dropna(subset=["lat", "lon"]).iterrows():
            ...
            if rainfall_coverage in ("Requested locations", "Marikina + Rizal Rain Watch"):
                folium.Marker(
                    location=[rrow["lat"], rrow["lon"]],
                    tooltip=(
                        f"{rrow['area']} | Next {forecast_rain_hours}h: "
                        f"{rrow.get('forecast_total_mm', 'No data')} mm"
                    ),
                    icon=folium.Icon(color=rrow.get("color", "blue"), icon="cloud")
                ).add_to(rain_group)
        rain_group.add_to(m)
    ...
    # Keep non-Metro rainfall stations visible without manual panning/zooming.
    if show_rain_layer and not rainfall_df.empty and rainfall_coverage != "Metro Manila":
        valid_map_points = rainfall_df.dropna(subset=["lat", "lon"])
        if not valid_map_points.empty:
            bounds = [
                [float(valid_map_points["lat"].min()), float(valid_map_points["lon"].min())],
                [float(valid_map_points["lat"].max()), float(valid_map_points["lon"].max())],
            ]
...
if show_rain_layer and not rainfall_df.empty:
    if rainfall_coverage == "Marikina + Rizal Rain Watch":
        ...
        requested_columns = [
            "area", "current_mmhr", "past_total_mm", "forecast_total_mm",
            "forecast_peak_mm", "forecast_peak_time", "possible_flood_risk", "trend"
        ]
        st.dataframe(
            rainfall_df[requested_columns].sort_values("forecast_total_mm", ascending=False),
            use_container_width=True
        )
